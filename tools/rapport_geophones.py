"""
Génère les figures et le tableau de synthèse du rapport de calibration des
géophones horizontaux (run du 2026-05-28).

Sortie : data/rapport_geophones_H_2026-05-28/figures/*.png  + résumé console.

Source des données :
- transfert de banc : data/transfert_banc_horizontal_<ts>.csv
- balayages géophone : data/<dut>/balayage_<dut>_horizontal_<ts>.csv (+ .npz onde par point)

Lancer :  python tools/rapport_geophones.py
"""
...
OUT = os.path.join(DATA, "rapport_geophones_H_2026-05-28", "figures")
...
BENCH_CSV = os.path.join(DATA, "transfert_banc_horizontal_2026-05-28_15-32-08.csv")

# (label, spec_key, sous-dossier, fichier balayage, couleur)
DUTS = [
    ("HG-6 HB",   "HG-6 HB",   "HG-6_HB",   "balayage_HG-6-HB_horizontal_2026-05-28_16-18-49.csv",   "#1f77b4"),
    ("VAS-H-200", "VAS-H-200", "VAS-H-200", "balayage_VAS-H-200_horizontal_2026-05-28_16-32-19.csv", "#d62728"),
    ("ST-2A (H)", "ST-2A (H)", "ST-2A-H",   "balayage_ST-2A-H_horizontal_2026-05-28_16-48-23.csv",   "#2ca02c"),
]
...
def load_onde(subdir, label_slug, freq, ts):
    fn = f"onde_{label_slug}_horizontal_{freq:g}Hz_{ts}.npz"
...
sweeps = [load_sweep(lbl, sub, fn) for (lbl, _k, sub, fn, _c) in DUTS]
colors = {lbl: c for (lbl, _k, _s, _f, c) in DUTS}
...
ax.set_title("Fonction de transfert du banc — axe horizontal\n"
...
ffit = np.logspace(np.log10(0.1), np.log10(50), 300)
for s in sweeps:
    c = colors[s["label"]]
    rel = s["reliable"]
...
ax.set_title("Fonction de transfert des géophones (réponse en vitesse)\n"
...
slugs = {"HG-6 HB": ("HG-6_HB", "HG-6-HB", "2026-05-28_16-18-49"),
         "VAS-H-200": ("VAS-H-200", "VAS-H-200", "2026-05-28_16-32-19"),
         "ST-2A (H)": ("ST-2A-H", "ST-2A-H", "2026-05-28_16-48-23")}
...
# ============================================ FIGURE 6 : ondes temporelles ST-2A @5Hz
npz = load_onde("ST-2A-H", "ST-2A-H", 5.0, "2026-05-28_16-48-23")
if npz is not None:
    ...
    a1.set_title("Signaux bruts simultanés — ST-2A (H) @ 5 Hz")
    g = npz["geo_wave"].astype(float)
    g = g - np.mean(g)
    fg = float(npz["geo_rate"])
    tg = np.arange(len(g)) / fg
    a2.plot(tg, g / 1e6, color="#2ca02c", lw=0.9)
